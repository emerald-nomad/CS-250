...
public:
    ...
    void Delete( const TK& key )
    {
        ...
        if (current->ptrLeft == nullptr && current->ptrRight == nullptr) {
        ...
        else if (current->ptrLeft == nullptr) {
            if (current == m_ptrRoot) {
                m_ptrRoot = current->ptrRight;
            }
            else if (isLeftNode == true) {
                parent->ptrLeft = current->ptrRight;
            }
            else {
                parent->ptrRight = current->ptrRight;
            }
        }
        else if (current->ptrRight == nullptr) {
            if (current == m_ptrRoot) {
                m_ptrRoot = current->ptrLeft;
            }
            else if (isLeftNode == true) {
                parent->ptrLeft = current->ptrLeft;
            }
            else {
                parent->ptrRight = current->ptrLeft;
            }
        }
        else {
            Node<TK, TD> *tempNode =  current->ptrRight;
            Node<TK, TD> *successor = current;
            Node<TK, TD> *successorParent = current;
            ...
            if (successor != current->ptrRight) {
                successorParent->ptrLeft = successor->ptrRight;
                successor->ptrRight = current->ptrRight;
            }
            ...
            if (current == m_ptrRoot) {
                ...
                current = parent->ptrLeft;
                parent->ptrLeft = successor;
            }
            else {
                current = parent->ptrRight;
                parent->ptrRight = successor;
            }
            
            successor->ptrLeft = current->ptrLeft;
        }
        
        current->ptrLeft = nullptr;
        current->ptrRight = nullptr;
        delete current;
    }
...
private:
    ...
    int GetHeight( Node<TK, TD>* ptrCurrent )
    {
        ...
        while (ptrCurrent->ptrLeft != nullptr) {
            leftHeight++;
            GetHeight(ptrCurrent->ptrLeft);
        }
        
        while (ptrCurrent->ptrRight!= nullptr) {
            rightHeight++;
            GetHeight(ptrCurrent->ptrRight);
        }
        
        return (leftHeight + rightHeight + 1); // temp
    }
